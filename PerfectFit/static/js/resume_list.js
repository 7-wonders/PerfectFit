...
async function showJobList(button) {
    const selectedOccupation = button.textContent.trim();  // 직군 이름
    const selectedOccupationId = button.value;  // 직군 ID (value에서 가져옴)
    ...
    try {
        ...
        const response = await instance.get(`/resume/job/${selectedOccupationId}`);
...
document.addEventListener("DOMContentLoaded", function() {
    // 직군을 선택하는 버튼들을 모두 가져옴
    const occupationButtons = document.querySelectorAll("#occupation-list-box button");

    // 직군 선택값 가져오기 (hidden input에서 값을 가져옵니다)
    const selectedOccupation = document.getElementById("resume-write-occupation").value;

    // 직군이 이미 선택된 상태라면, 직군에 해당하는 버튼을 찾기
    const selectedButton = Array.from(occupationButtons).find((button) => {
        return button.value === selectedOccupation;
    });

    // 직군이 이미 선택된 상태라면, 직업 목록을 한번 호출
    if (selectedButton) {
        // 직업 목록을 호출하는 함수
        showJobList(selectedButton);
    }
});
...
function setArray(arrayType) {
    ...
    if (job !== "직업 선택") params.append("job", job);
    if (occupation !== "직군 선택") params.append("occupation", occupation);
    ...
    if (currentParams.has("keyword")) {
        params.append("keyword", currentParams.get("keyword"));
    }
...
document.addEventListener("DOMContentLoaded", function() {
    document.getElementById("search-input").addEventListener("keydown", function(event) {
        if (event.key === "Enter") {
            ...
            if (currentParams.has("keyword")) {
                params.delete("keyword", currentParams.get("keyword"));
            }
